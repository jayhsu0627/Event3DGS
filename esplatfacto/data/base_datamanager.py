    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.train_dataparser_outputs is not None:
            ...
            pre_cameras = self.train_dataparser_outputs.pre_cameras
            ...
            if len(cameras) > 1:
                for i in range(1, len(cameras)):
                    if cameras[0].width != cameras[i].width or cameras[0].height != cameras[i].height:
                        ...
                        break
            if len(pre_cameras) > 1:
                for i in range(1, len(pre_cameras)):
                    if pre_cameras[0].width != pre_cameras[i].width or pre_cameras[0].height != pre_cameras[i].height:
                        CONSOLE.print("Variable resolution, using variable_res_collate")
                        self.config.collate_fn = variable_res_collate
                        break
        super().__init__()
